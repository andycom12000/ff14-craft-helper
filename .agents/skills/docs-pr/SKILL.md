...
If the local feature branch was created in an isolated worktree (under `.Codex/worktrees/`), remove the worktree first or `--delete-branch` will refuse:
...
- **Release changelog bump tied to a tag** — that's part of the release workflow (see `src/views/ChangelogView.vue` + `.Codex/scripts/check-changelog.sh`), not a standalone docs PR.
