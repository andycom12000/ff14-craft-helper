...
const changelog: Entry[] = [
  {
    ...
    date: "2026-08-12",
    highlights: [
      "【宇宙探索的 D／C／B 級配方會依你的等級換算了】這類任務配方沒有固定難度 —— 接取時遊戲會依你當下的職業等級重算難度、品質上限與耐久，而本站以前一律當成 Lv100 配方處理。對滿等玩家沒差（在 100 級換算是恆等變換，畫面一個字都不會變），但拿宇宙探索練級的人看到的數字全錯：一個 Lv90 裁縫做「統一規格的棉線」，遊戲裡是 進度 2135 ／ 品質 3456 ／ 耐久 40，本站卻顯示 4026 ／ 5760 ／ 40。最麻煩的是它「看起來很正常」，求解器照著錯的難度給你一份手法，要真的進遊戲做下去才會發現不對。現在難度、品質上限、耐久、星數、配方等級、以及進度與品質的換算係數全部改用換算後的值，模擬器、批量製作、鑲嵌建議、可行性預篩、素材 HQ 的初始品質、等級門檻判定一路吃到底。你的作業、加工、CP 不受影響 —— 是配方向你靠攏，不是你被壓低，所以拿滿裝去做低階同步任務確實會變輕鬆，跟遊戲一致",
      "【換算的唯一依據是你的裝備組等級】沒有手動指定同步等級的入口，工具也不會從你的屬性反推真實等級。這代表：如果你實際是 Lv50、但裝備組等級停在預設的 100，你會拿到 Lv100 的難度、算出一份你做不到的手法，而且畫面上不會有任何異狀（降級標示也不會出現，因為在工具眼中沒有發生降級）。用宇宙探索練級前，請先把該職業的裝備組等級填對",
      "【實際降級時會標示出來】當換算後的等級低於配方原始等級，模擬器的製作狀態列會出現「等級同步 Lv.94　原始 Lv.100」，並列兩個等級，讓你能跟遊戲內配方手帳上寫的 Lv100 對得起來。沒有實際降級（例如滿等玩家）就不會顯示任何東西",
      "【只有會同步的配方受影響】判斷依據是配方自己帶的「可調整職業等級」欄位，不是「這張配方是不是宇宙探索的」—— 宇宙探索的 A／EX／EX+ 級是固定等級的一般配方，不會被換算。全遊戲符合條件的共 768 張，全部是 D／C／B 級任務用配方。一般配方的行為完全不變",
      "【已知限制】本次只做配方層的等級換算。宇宙探索的任務層（一個任務綁多張配方、銀星／金星的評分門檻）不在範圍內 —— 評分相關的遊戲資料表目前連欄位名稱都還沒被社群還原。同步任務中的臨時技能（星際穩手、素材奇蹟）也還沒接，這兩者與等級換算彼此獨立，會另外評估",
      "【內部】修正 GA 的 rlv 維度：同步後的配方帶著換算後的配方等級進到求解事件，會讓這 768 張配方散進 recipes.json 裡根本不存在的 rlv 分組，打亂 /admin/ga 的工具使用分佈圖。現在同步後的配方會另外保留原始配方等級供埋點使用，製作運算仍用換算後的值。這一區只有管理端看得到，不影響一般使用",
    ],
  },
  {
    version: "v2.23.0",
    date: "2026-08-08",
    highlights: [
      "【補齊當前世代食藥】食物新增 4 種、藥水新增 1 種，把 7.x 這一輪該有的製作食藥補完：椒麻鰻魚（il750，目前 CP 上限最高的製作食物，CP +26%／上限 100）、酸檸檬醃魚（il720，CP +26%／上限 96，作業精度 +5%／上限 150）、巧克力奶油蛋糕（il657，作業精度特化 +9%／上限 240）、鮭魚乾（il670，加工精度特化 +9%／上限 215），以及名匠藥液（il665，作業精度 +3%／上限 63）—— 補上這顆之後，三種藥液（名匠／巨匠／魔匠）終於湊齊作業、加工、CP 三個方向。原本清單只收了「CP +26%」那一系，所以既漏掉 7.2 之後新增的兩檔 CP 食物，也整條缺了作業／加工特化系。所有數值（含 NQ）都對過 7.55 遊戲資料逐欄驗證。食物選單改依物品等級排序",
      "【卡進度的配方也能拿到食藥建議了】「自動評估食藥」在正式試算前會先判斷「有沒有任何食藥救得了這張配方」，而這個判斷以前只挑「加工＋CP 合計最大」的那一組來試。結果是：卡在進度（作業精度不足）的配方永遠過不了這關、直接回報沒有建議，作業精度取向的食藥也從來沒被試過。現在會分別針對加工、CP、作業精度、以及加工＋CP 合計各挑一組來試，任一組行得通就繼續往下算。同時修掉一個連帶問題 —— 新增鮭魚乾（加工 +215）之後，「加工＋CP 合計」的贏家會被它搶走，反而讓 CP 掉了 92，卡 CP 的配方會從「有建議」變成「沒建議」；四個方向分開評估之後不會再有這種互相蓋掉的情況。另外評估過程中按取消現在會立即停下，不必等剩餘的求解跑完",
      "【已知限制】椒麻鰻魚的繁體中文名稱為暫定 —— 繁中客戶端的資料目前仍停在 7.2，官方譯名還沒出來，此處採國服官方譯名的繁體寫法。若你把這個名稱貼進遊戲內市場板搜尋不到，請改用英文名 All i Pebre",
      "【內部】/admin/ga 決策儀表板本期落地 20 個 commit（#212–#230）：版面重建為兩區、鏡像分級與三階字級，砍掉失效圖表與管線查詢；補上 human/machine 求解判別並套用到所有 solver 分母、Universalis 真實失敗與無上架分流、hero 分母修正；新增門檻表與單期 verdict 判定引擎、趨勢檔與 streak／熄火判定、本期待辦 ledger 與趨勢三連（數值／設限 WoW／7 日走勢）。這一區只有管理端看得到，不影響一般使用",
    ],
  },
  {
    version: "v2.22.2",
    date: "2026-07-28",
    highlights: [
      "【遊戲資料更新】繁中資料同步至 2026.07.22 最新上游版本：新增 1,706 筆物品、修正 16 個既有譯名，沒有移除資料；國際版上游也已檢查至最新版，英／日物品、配方與製作等級表內容無異動；简中資料同步新增 413 筆並修正 34 筆譯名",
    ],
  },
  {
    version: "v2.22.1",
